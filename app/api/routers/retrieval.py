...
def ingest_public_docs(db: Session = Depends(get_db_session)) -> RetrievalIngestResponse:
...
def ingest_internal_docs(db: Session = Depends(get_db_session)) -> RetrievalIngestResponse:
...
def reindex_docs(db: Session = Depends(get_db_session)) -> RetrievalReindexResponse:
...
def build_index(db: Session = Depends(get_db_session)) -> RetrievalReindexResponse:
    out = service.build_index(db=db)
    return RetrievalReindexResponse(**out)


@router.get("/docs", response_model=RetrievalDocsResponse)
def get_docs(db: Session = Depends(get_db_session)) -> RetrievalDocsResponse:
